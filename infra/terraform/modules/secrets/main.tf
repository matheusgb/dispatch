...
# DISPATCH_JWT_SECRET. Contra AWS real, KMS teria rotação automática de
...
resource "aws_kms_key" "jwt" {
  description             = "chave usada para cifrar o segredo do JWT do dispatch"
...
resource "aws_kms_alias" "jwt" {
  name          = "alias/dispatch-${var.environment}-jwt"
