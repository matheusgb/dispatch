...
| --- | --- | --- |
| `dispatch.delivery-events` | todo evento de lifecycle (criada, pronta, ofertada, atribuída, recusada, expirada, coletada, entregue) | `delivery_id` |
| `dispatch.tracking-positions` | cada posição de GPS aceita | `delivery_id` |
...
throughput. Com o volume deste laboratório (LunchRush na casa de dezenas a
...
- réplicas úteis de um consumer group são limitadas a 3 (o número de
  partições): a 4ª réplica de `dispatch-worker` ficaria ociosa. Isso é
