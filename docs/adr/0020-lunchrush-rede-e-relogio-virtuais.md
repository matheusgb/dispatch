# ADR 0020: LunchRush ganha rede e relógio virtuais reaproveitando os handlers reais

## Contexto

O tier 5 pede que o LunchRush ganhe relógio e rede virtuais (drop, atraso,
...
`cmd/lunchrush/netfault.go` decide, a partir do mesmo `rand.Rand` seedado
...
dois relatórios são **idênticos** (`docs/benchmarks/tier-5-lunchrush-netfault/`,
...
`lunchrush-<seed>-<index>` devolveu, corretamente, a mesma entrega já
criada na primeira execução — mas essa entrega já tinha avançado de
estado, então `/ready` respondeu `409` (`dispatch: entrega não está em
...
- **Partição control plane / data plane explícita**: o roadmap pede isso
  ...
  caso de primeira classe. Uma partição de rede *dentro do LunchRush*
  ...
  LunchRush nesta sessão por tempo;
- **Failover no pior momento coordenado com o LunchRush**: o runbook de
  ...
  LunchRush (o "pior momento") fica como próximo passo natural, não
...
- **Reimplementar a máquina de estados e a lógica de dedup dentro do
  LunchRush, para simular sem precisar de um servidor real no ar**:
  rejeitada explicitamente pelo próprio roadmap ("evitando um simulador
  que prove um sistema diferente do deploy"). O LunchRush continua
