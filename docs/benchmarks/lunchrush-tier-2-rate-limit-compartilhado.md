# Relatório LunchRush — identidade compartilhada esgota o rate limit

**Nota de leitura:** os 429 abaixo não são bug do rate limit, são uma
limitação real do próprio LunchRush: todas as ordens simuladas enviam GPS
autenticadas como o mesmo caller (`lunchrush`), então elas competem pelo
...
`lunchrush-tier-2-golden.md`, que usa concorrência 1 e fica sem erros.
