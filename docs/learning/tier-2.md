...
3. **Que evidência mudou minha opinião?** Nenhuma mudou a decisão de
   arquitetura, mas o teste `TestDispatch_CourierCannotHoldTwoActiveDeliveries`
...
4. **Onde o sistema quebrou?** No próprio LunchRush, não no delivery-api:
...
5. **Como diagnostiquei?** Lendo a amostra de falhas do relatório do
   LunchRush (`status 429`) e comparando com o rate limit configurado
...
6. **Qual solução considerei e rejeitei?** Aumentar o rate limit global só
   para o LunchRush passar sem erro. Rejeitei porque escondia o achado
...
8. **O que eu faria diferente em um sistema real?** Geraria um token por
   ...
   "serviço simulado". O custo de fazer isso direito no LunchRush é baixo
...
9. **Qual é o próximo limite conhecido?** Nenhuma medição de stress ou
   breakpoint ainda: os números atuais vêm de smoke tests e do LunchRush em
