// Package lunchrush seleciona candidatos, oferece, expira e atribui entregas
...
var (
	...
	ErrNotOffered = errors.New("lunchrush: entrega não está em oferta")
	...
	ErrCourierAlreadyActive = errors.New("lunchrush: entregador já possui entrega ativa")
	// ErrNotReadyForLunchRush indica que a entrega não estava disponível
	// para receber uma oferta.
	ErrNotReadyForLunchRush = errors.New("lunchrush: entrega não está pronta para despacho")
	// ErrNotCreated indica que a entrega não estava em created ao tentar
	// marcá-la como pronta para despacho.
	ErrNotCreated = errors.New("lunchrush: entrega não está em created")
	// ErrUnexpectedState indica que a entrega não estava no estado de
	// origem esperado para a transição simples solicitada.
	ErrUnexpectedState = errors.New("lunchrush: entrega não está no estado esperado para esta transição")
)
...
// MarkReadyForLunchRush move created -> ready_for_lunchrush. No tier 1 isso é
// uma chamada explícita; a partir do tier 3 o lunchrush-worker assume essa
// decisão sozinho, a partir de uma regra de seleção.
func (s *Service) MarkReadyForLunchRush(ctx context.Context, deliveryID string) error {
	now := s.clock.Now()
	...
	`, delivery.ReadyForLunchRush, now, deliveryID, delivery.Created)
	if err != nil {
		return err
	}
	...
	if err := recordTransition(ctx, tx, deliveryID, delivery.Created, delivery.ReadyForLunchRush, now); err != nil {
		return err
	}
	if err := enqueueDeliveryEvent(ctx, tx, deliveryID, topics.KindDeliveryReadyForLunchRush); err != nil {
		return err
	}
...
// Offer move ready_for_lunchrush -> offered com um prazo de expiração. Só
...
func (s *Service) Offer(ctx context.Context, deliveryID string, ttl time.Duration) error {
	...
	`, delivery.Offered, now, now.Add(ttl), deliveryID, delivery.ReadyForLunchRush)
	if err != nil {
		return err
	}
	if tag.RowsAffected() == 0 {
		return ErrNotReadyForLunchRush
	}
	if err := recordTransition(ctx, tx, deliveryID, delivery.ReadyForLunchRush, delivery.Offered, now); err != nil {
		return err
	}
...
// Decline devolve a entrega para ready_for_lunchrush depois de uma recusa
...
// ready_for_lunchrush, toda oferta cujo prazo já passou segundo o relógio
...
// recycle move offered -> via (declined ou expired) -> ready_for_lunchrush
...
func (s *Service) recycle(ctx context.Context, deliveryID string, via delivery.State) error {
	...
	if _, err := tx.Exec(ctx, `
		...
		WHERE id = $3
	`, delivery.ReadyForLunchRush, now, deliveryID); err != nil {
		return err
	}
	if err := recordTransition(ctx, tx, deliveryID, via, delivery.ReadyForLunchRush, now); err != nil {
		return err
	}
	if err := enqueueDeliveryEvent(ctx, tx, deliveryID, topics.KindDeliveryReadyForLunchRush); err != nil {
		return err
